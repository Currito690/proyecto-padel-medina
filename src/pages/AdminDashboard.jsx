...
const MOCK_CLIENTS = ['Carlos M.', 'Ana G.', 'Pedro R.', 'Laura S.', 'Miguel F.', 'Sofía B.'];
...
const generateInitialSlots = () => {
  const slots = {};
  COURTS_DATA.forEach(({ id }) => {
    slots[id] = {};
    TIMES.forEach(time => {
      const rand = Math.random();
      if (rand < 0.28) {
        slots[id][time] = { status: 'booked', client: MOCK_CLIENTS[Math.floor(Math.random() * MOCK_CLIENTS.length)] };
      } else if (rand < 0.40) {
        slots[id][time] = { status: 'blocked' };
      } else {
        slots[id][time] = { status: 'available' };
      }
    });
  });
  return slots;
...
const AdminDashboard = () => {
  ...
  const [courts, setCourts] = useState(() => COURTS_DATA.map(c => ({ ...c, active: true })));
  const [activeSlot, setActiveSlot] = useState(null);

  const handleSlotClick = (courtId, time) => {
    setActiveSlot(prev =>
      prev?.courtId === courtId && prev?.time === time ? null : { courtId, time }
    );
  };
  ...
  const handleAction = (action) => {
    ...
    setSlots(prev => {
      ...
      if (action === 'reserve') next[courtId][time] = { status: 'booked', client: 'Admin (Reserva directa)' };
      else if (action === 'block') next[courtId][time] = { status: 'blocked' };
      else next[courtId][time] = { status: 'available' };
      return next;
    });
    setActiveSlot(null);
  };

  const cancelBooking = (courtId, time) => {
    setSlots(prev => ({
      ...prev,
      [courtId]: { ...prev[courtId], [time]: { status: 'available' } },
    }));
  };
  ...
  COURTS_DATA.forEach(court => {
    TIMES.forEach(time => {
      ...
      if (slot?.status === 'booked') {
        allBookings.push({ ...court, time, client: slot.client });
      }
    });
  });
      ...
      <div className="admin-wrap">
        ...
        <div className="admin-header">
          <div style={{ maxWidth: '1060px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button
              onClick={logout}
              style={{ padding: '0.5rem 1rem', border: '1.5px solid #E2E8F0', borderRadius: '0.625rem', background: 'white', color: '#475569', fontFamily: 'inherit', fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}
            >
              Salir
            </button>
          </div>
                  ...
                  return (
                    <div key={court.id} style={{ backgroundColor: 'white', borderRadius: '1rem', border: '1px solid #E2E8F0', overflow: 'hidden', opacity: court.active ? 1 : 0.5, boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
                      ...
                      <div style={{ background: court.gradient, padding: '0.75rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        {!court.active && (
                          <span style={{ fontSize: '0.65rem', fontWeight: 700, backgroundColor: 'rgba(0,0,0,0.25)', color: 'white', padding: '0.2rem 0.5rem', borderRadius: '999px' }}>INACTIVA</span>
                        )}
                      </div>
                          ...
                          {TIMES.map(time => {
                            ...
                            return (
                              <button
                                ...
                                style={{
                                  padding: '0.5rem 0.2rem', borderRadius: '0.5rem',
                                  border: `1.5px solid ${c.borderColor}`, backgroundColor: c.backgroundColor, color: c.color,
                                  fontFamily: 'inherit', fontWeight: 700, fontSize: '0.68rem',
                                  textAlign: 'center', cursor: court.active ? 'pointer' : 'not-allowed',
                        ...
                        {isCourtActive && selectedSlotData && (
                          <div style={{ backgroundColor: '#F8FAFC', borderRadius: '0.75rem', border: '1.5px solid #E2E8F0', padding: '0.875rem', marginTop: '0.5rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                              <div>
                                <p style={{ margin: 0, fontWeight: 800, color: '#0F172A', fontSize: '0.875rem' }}>
                                  {activeSlot.time} – {TIME_END[activeSlot.time]}
                                </p>
                                <p style={{ margin: '0.15rem 0 0', fontSize: '0.775rem', color: '#64748B' }}>
                                  {selectedSlotData.status === 'booked'
                                    ? `Cliente: ${selectedSlotData.client}`
                              ...
                              <button
                                ...
                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8', padding: '0.2rem', fontSize: '1rem', lineHeight: 1 }}
                              >✕</button>
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                              {selectedSlotData.status === 'available' && (
                                <>
                                  <button
                                    ...
                                    style={{ padding: '0.5rem 0.875rem', borderRadius: '0.5rem', border: 'none', backgroundColor: '#16A34A', color: 'white', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}
                                  >
                                    ✓ Reservar (gratis)
                                  </button>
                                  <button
                              ...
                              {selectedSlotData.status === 'booked' && (
                                <button
                                  ...
                                  style={{ padding: '0.5rem 0.875rem', borderRadius: '0.5rem', border: 'none', backgroundColor: '#DC2626', color: 'white', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}
                                >
                                  ✕ Cancelar reserva
                                </button>
                              )}
                              {selectedSlotData.status === 'blocked' && (
                                <button
                                  ...
                                  style={{ padding: '0.5rem 0.875rem', borderRadius: '0.5rem', border: 'none', backgroundColor: '#0EA5E9', color: 'white', fontFamily: 'inherit', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}
                                >
                                  🔓 Desbloquear
                                </button>
                              )}
          ...
          {activeTab === 'bookings' && (
            <div>
              <p className="section-label" style={{ marginBottom: '1rem' }}>
                Reservas activas — {formatDate(selectedDate)}
              </p>
              {allBookings.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '3.5rem 1rem', color: '#94A3B8' }}>
                  ...
                  <p style={{ margin: '0.4rem 0 0', fontSize: '0.85rem' }}>Cambia la fecha en la pestaña Horario</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                  {allBookings.map(b => (
                    <div
                      key={`${b.id}-${b.time}`}
                      style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', backgroundColor: 'white', borderRadius: '0.875rem', padding: '0.875rem', border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ margin: '0.1rem 0 0', fontSize: '0.775rem', color: '#64748B' }}>
                          {b.time} – {TIME_END[b.time]} · {b.client}
                        </p>
                      </div>
                      <button
                        onClick={() => cancelBooking(b.id, b.time)}
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {courts.map(court => (
                  <div
                    key={court.id}
                    style={{ display: 'flex', alignItems: 'center', gap: '1rem', backgroundColor: 'white', borderRadius: '1rem', padding: '1rem', border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, color: court.active ? '#16A34A' : '#94A3B8', transition: 'color 0.2s' }}>
                        {court.active ? 'Activa' : 'Inactiva'}
                      </span>
                      {/* Toggle switch */}
                      <button
                        onClick={() => toggleCourt(court.id)}
                        aria-label={court.active ? 'Desactivar pista' : 'Activar pista'}
              ...
              <div style={{ marginTop: '1.5rem', padding: '1rem', backgroundColor: '#F0FDF4', borderRadius: '0.875rem', border: '1px solid #86EFAC' }}>
                <p style={{ margin: 0, fontSize: '0.8rem', color: '#15803D', fontWeight: 500, lineHeight: 1.55 }}>
                  Las pistas desactivadas no aceptan nuevas reservas de clientes. Las reservas existentes se mantienen activas.
                </p>
              </div>
            </div>
          )}
